...
Golden fixtures pin Finstack's valuation outputs against externally sourced
reference values (Bloomberg screens, QuantLib, closed-form formulas). Fixtures
live under `data/` and use the strict `finstack.golden/2` schema. Both the Rust
runner (`tests/golden/`) and the Python bindings layer
(`finstack-py/tests/golden/`) load the same JSON files and must agree.

## Schema (`finstack.golden/2`)
...
[`finstack-py/tests/golden/schema.py`](../../../../finstack-py/tests/golden/schema.py).
...
- `instrument` — a `finstack.instrument/1` envelope.
