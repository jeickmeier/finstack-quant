...
) -> finstack_core::Result<Vec<CachedFlow>> {
    let dc = disc.day_count();
    inputs
        .flows
        .iter()
        .filter(|(d, _)| *d > inputs.settlement)
        .map(|(d, amt)| -> finstack_core::Result<CachedFlow> {
...
fn has_credit_curve<I: CurveDependencies>(instrument: &I) -> finstack_core::Result<bool> {
