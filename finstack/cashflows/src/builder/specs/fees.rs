...
impl FeeTier {
    ...
    /// # Errors
    ///
    /// Returns [`finstack_core::Error::Validation`] when `threshold` is NaN,
    /// infinite, or outside the `Decimal` representable range.
    pub fn from_bps(threshold: f64, bps: Bps) -> finstack_core::Result<Self> {
...
/// # Errors
///
/// Returns [`finstack_core::Error::Validation`] when tier thresholds are not
/// strictly ascending (the tier-selection logic relies on this ordering).
pub fn evaluate_fee_tiers(
    tiers: &[FeeTier],
    utilization: Decimal,
) -> finstack_core::Result<Decimal> {
