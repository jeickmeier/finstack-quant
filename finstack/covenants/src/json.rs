...
use finstack_core::dates::parse_iso_date;
...
pub fn evaluate_engine_json(engine_json: &str, metrics_json: &str, as_of: &str) -> Result<String> {
    ...
    serde_json::to_string(&reports)
        .map_err(|e| finstack_core::Error::Validation(format!("Serialize covenant reports: {e}")))
}
...
pub fn lbo_standard_json(
    ...
    .map_err(|e| finstack_core::Error::Validation(format!("Serialize covenant template: {e}")))
}

/// Covenant-lite package as JSON.
pub fn cov_lite_json(max_leverage: f64, max_senior_leverage: f64) -> Result<String> {
    serde_json::to_string(&templates::cov_lite(max_leverage, max_senior_leverage))
        .map_err(|e| finstack_core::Error::Validation(format!("Serialize covenant template: {e}")))
}

/// Real-estate covenant package as JSON.
pub fn real_estate_json(min_dscr: f64, min_debt_yield: f64, max_ltv: f64) -> Result<String> {
    serde_json::to_string(&templates::real_estate(min_dscr, min_debt_yield, max_ltv))
        .map_err(|e| finstack_core::Error::Validation(format!("Serialize covenant template: {e}")))
}
