...
Analysis, reporting, templates, and runtime extensions on top of `finstack-statements`.

## Where it fits

| Need | Crate |
|------|-------|
| Build and evaluate statement models | `finstack-statements` |
| Scenarios, DCF, covenants, reports, templates, extensions | `finstack-statements-analytics` |
| Instrument pricing and covenant engines | `finstack-valuations` |
| Dates, money, curves, core types | `finstack-core` |
...
**Credit and covenants** — `compute_credit_context()` derives coverage and leverage from statement results plus capital-structure cashflows. `forecast_breaches()` and `forecast_covenant(s)()` bridge into the `finstack-valuations` covenant engine.
...
Most types re-export from `finstack_statements_analytics::analysis::*`.
...
cargo doc -p finstack-statements-analytics --no-deps
```
