...
import { labHref } from '@/lib/curriculum';
...
export function LabCard({ title = 'Open the companion lab', href, description, children, notebook }: {
  ...
  const url = href ?? (notebook ? labHref(notebook) : '/labs');
  const isPending = url.startsWith('/labs/') && !labSource.getPage(url.replace(/^\/labs\//, '').split('/'));
  if (isPending) {
    return (
      <div className="lab-card lab-pending">
        <FlaskConical size={20} aria-hidden="true" />
        <span>
          <span className="section-eyebrow">Companion notebook · awaiting validation</span>
          <strong>{title}</strong>
  ...
  return (
    <Link href={url} className="lab-card">
      <FlaskConical size={20} aria-hidden="true" />
      <span>
        <span className="section-eyebrow">Companion notebook</span>
        <strong>{title}</strong>
        {description && <span>{description}</span>}
        {children}
      </span>
...
function renderSnippetAsset(asset: string, block: string) {
  ...
  if (/\.html$/i.test(asset)) {
    return (
      <div className="output-report" key={asset}>
        <iframe src={url} title={`Calculation report from ${block}`} sandbox="allow-scripts" loading="lazy" />
        <a href={url} target="_blank" rel="noreferrer">Open report full width <ArrowUpRight size={14} aria-hidden="true" /></a>
      </div>
    );
  }
  if (/\.(png|jpg|jpeg|svg|webp)$/i.test(asset)) {
    return (
      <a className="output-image" href={url} key={asset}>
        <img src={url} alt={`Calculation figure from ${block}`} loading="lazy" />
      </a>
    );
  }
  return <a href={url} key={asset} className="output-download">Download {asset.split('/').at(-1)}</a>;
}
...
export function ExecutedOutput({ lesson, block }: { lesson: string; block: string }) {
  ...
  return (
    <figure className="executed-output">
      ...
      {result ? (
        <>
          ...
          {result.assets.map((asset) => renderSnippetAsset(asset, block))}
        </>
      ) : (
