...
import { getLessonLabs, getLessons, labHref, labTitle } from '@/lib/curriculum';
...
export default function LabsPage() {
  ...
  return <DocsPage full footer={{ enabled: false }}><div id="main-content"><p className="section-eyebrow">Read · run · investigate</p><DocsTitle>Companion labs</DocsTitle><p className="page-intro">The working notebooks behind the program. Open a rendered lab, then download it to explore the calculations in your own workspace.</p></div><div className="lab-directory">{labs.map((lab) => <Link href={labHref(lab)} key={lab}><span><strong>{labTitle(lab)}</strong><small>{lessons.filter((lesson) => getLessonLabs(lesson).includes(lab)).map((lesson) => lesson.id).join(' · ')}</small></span><ArrowUpRight size={18} aria-hidden="true" /></Link>)}</div></DocsPage>;
}
