...
The first calculation reconciles dollar PV to the reported expected variance. The strip then agrees with that metric to numerical precision, while the flat-volatility limit uses a separate discretization tolerance.
