...
//! delegate all calculations to `finstack_portfolio::performance`.
...
fn twrr_modified_dietz(period_json: &str) -> PyResult<Option<f64>> {
    let period: finstack_portfolio::TwrrPeriod = serde_json::from_str(period_json)
...
fn twrr_linked(returns_json: &str, horizon_years: f64) -> PyResult<Option<String>> {
...
fn mwr_xirr(cashflows_json: &str) -> PyResult<f64> {
    let cashflows: Vec<finstack_portfolio::DatedCashflow> = serde_json::from_str(cashflows_json)
        .map_err(|err| serde_json_to_py(err, "invalid MWR cashflows JSON"))?;
    finstack_portfolio::mwr_xirr_from_cashflows(&cashflows).map_err(core_to_py)
}
