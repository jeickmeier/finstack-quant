...
function model(values, forecast) {
  return {
    id: 'statement-production', schema_version: 1,
    periods: [1, 2, 3, 4].map((q) => ({
      id: `2025Q${q}`, start: `2025-${String(q * 3 - 2).padStart(2, '0')}-01`,
      end: q === 4 ? '2026-01-01' : `2025-${String(q * 3 + 1).padStart(2, '0')}-01`,
      is_actual: q === 1,
    })),
    nodes: { revenue: { node_id: 'revenue', node_type: 'value', values, ...(forecast ? { forecast } : {}) } },
  };
}

for (const forecast of [
  { method: 'growth_pct', params: { rate: 0.1 } },
  { method: 'seasonal', params: { historical: [100, 60, 120, 80, 100, 60, 120, 80], season_length: 4, mode: 'additive' } },
]) {
...
test('formula residual checks accept the tolerance boundary and reject larger residuals', () => {
  ...
  const suite = { name: 'residual', builtin_checks: [], formula_checks: [{
    id: 'residual', name: 'Residual', category: 'accounting_identity', severity: 'error',
    formula: 'revenue', message_template: 'residual {period}', tolerance: 0.1,
  }] };
  const report = statements_analytics.runChecks(JSON.stringify(m), JSON.stringify(suite));
  assert.deepEqual(report.results[0].findings.map((finding) => finding.period), ['2025Q4']);
  suite.formula_checks[0].formula = 'revenue <= 0.1';
  suite.formula_checks[0].tolerance = null;
  assert.deepEqual(statements_analytics.runChecks(JSON.stringify(m), JSON.stringify(suite)).results[0].findings.map((f) => f.period), ['2025Q4']);
});
...
test('retained earnings accepts the elected inflow-positive dividend convention', () => {
  ...
  m.nodes.re = { node_id: 're', node_type: 'value', values: { '2025Q1': 1000, '2025Q2': 1080, '2025Q3': 1160, '2025Q4': 1240 } };
  m.nodes.div = { node_id: 'div', node_type: 'value', values: { '2025Q1': -20, '2025Q2': -20, '2025Q3': -20, '2025Q4': -20 } };
  const suite = { name: 'signed-dividends', formula_checks: [], builtin_checks: [{
    type: 'retained_earnings_reconciliation', retained_earnings_node: 're', net_income_node: 'revenue',
    dividends_node: 'div', dividends_sign_convention: 'inflow_positive',
  }] };
  const report = statements_analytics.runChecks(JSON.stringify(m), JSON.stringify(suite));
...
test('goal seek rejects a discontinuity that brackets the target without reaching it', () => {
  ...
  m.nodes.target = { node_id: 'target', node_type: 'calculated', formula_text: 'if(revenue < 0, 0, 1)' };
  assert.throws(() => statements_analytics.goalSeek(JSON.stringify(m), 'target', '2025Q4', 0.5, 'revenue', '2025Q4', true, -1, 1), /residual|target.*tolerance/i);
});
