...
/// (e.g. [`crate::monte_carlo::discretization::QeHeston`]) or when the process does not
...
impl McEngine {
    ...
    /// may include a captured [`crate::monte_carlo::paths::PathDataset`].
    ///
    /// # Arguments
    ///
    ...
    /// raised by [`ProcessParams::validate`](crate::monte_carlo::paths::ProcessParams::validate).
    ...
    pub(crate) fn finalize_captured_result(
        ...
        let run = crate::monte_carlo::results::RunMetadata {
            seed: None,
        ...
        F: Payoff,
    {
        ...
        let chunk_results: Vec<Result<OnlineStats>> = chunks
            ...
            .map(|range| {
                ...
                for path_id in range.clone() {
                    let mut path_rng = rng.split(path_id as u64).ok_or_else(|| finstack_quant_core::Error::Validation("RandomStream does not support stream splitting; use a splittable generator such as PhiloxRng or run in serial mode without per-path splitting".to_string()))?;
