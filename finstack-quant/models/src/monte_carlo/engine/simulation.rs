...
use crate::monte_carlo::paths::{PathPoint, SimulatedPath};
...
/// fractional-noise wrapper in [`crate::monte_carlo::engine_fractional`].
///
...
impl McEngine {
    ...
    /// [`crate::monte_carlo::pricer::path_dependent`]) can drive one path at a time with a
        ...
        F: Payoff,
    {
        ...
        if cashflow_amounts.len() >= 2 {
            ...
            if let Ok(rate) = finstack_quant_core::cashflow::irr(&cashflow_amounts, None) {
