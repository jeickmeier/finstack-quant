...
use finstack_quant_core::cashflow::{xirr, xirr_with_daycount_ctx};
...
fn from_grid_opts_default_matches_from_grid() {
    ...
    for &e in &exp {
        for &k in &strikes {
            assert!(
                (finstack_quant_models::volatility::get_surface_vol(&a, e, k).unwrap()
                    - finstack_quant_models::volatility::get_surface_vol(&b, e, k).unwrap())
                .abs()
                    < 1e-15
            );
        }
...
fn vol_surface_builder_matches_grid_constructor() {
    ...
    for &expiry in &expiries {
        for &strike in &strikes {
            assert_eq!(
                finstack_quant_models::volatility::get_surface_vol(&built, expiry, strike)
                    .expect("builder value"),
                finstack_quant_models::volatility::get_surface_vol(&direct, expiry, strike)
                    .expect("direct value")
            );
        }
    }
...
fn from_grid_opts_preserves_axis_and_interpolation_mode() {
    ...
    for &e in &exp {
        for &k in &strikes {
            assert!(
                finstack_quant_models::volatility::get_surface_vol(&surface, e, k)
                    .unwrap()
                    .is_finite()
            );
        }
...
fn value_checked_matches_value_clamped_on_interior() {
    ...
    for (e, k) in test_pts {
        let a = finstack_quant_models::volatility::get_surface_vol(&surface, e, k).unwrap();
        let b = finstack_quant_models::volatility::get_surface_vol_clamped(&surface, e, k);
        assert!(
            (a - b).abs() < 1e-15,
