...
impl PricerRegistry {
    ...
    fn price_with_metrics_impl(
        ...
        let metric_as_of = base_result.as_of;

        if !needs_split || !instrument.has_custom_metrics_equivalent() {
            let metric_measures = crate::instruments::common_impl::helpers::compute_metrics_dyn(
        ...
        let mut metric_measures = if !spread_metrics.is_empty() {
            ...
            crate::instruments::common_impl::helpers::compute_metrics_dyn(
                spread_instrument,
                Arc::clone(&market_arc),
                metric_as_of,
                base_result.value,
        ...
        if !risk_metrics.is_empty() {
            let risk_measures = crate::instruments::common_impl::helpers::compute_metrics_dyn(
                std::sync::Arc::from(instrument.clone_box()),
                market_arc,
                metric_as_of,
                base_result.value,
            ...
            for (k, v) in risk_measures {
                metric_measures.insert(k, v);
            }
        }
