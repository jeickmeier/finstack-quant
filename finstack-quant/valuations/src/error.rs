...
//! error types, following the same pattern as [`finstack_quant_core::Error`] wrapping
//! [`finstack_quant_core::InputError`].
//!
//! # Design
//!
//! ```text
...
//! all other Finstack Quant crates (portfolio, statements, scenarios, io).
...
//!
//! The error module layout across Finstack Quant crates follows this convention:
//!
...
//! ).into();
//!
//! // And the unified type converts into finstack_quant_core::Error
...
/// error through a single type, consistent with [`finstack_quant_core::Error`]
/// wrapping [`finstack_quant_core::InputError`].
///
...
mod tests {
    ...
    fn core_error_wraps_into_unified() {
        let core = finstack_quant_core::Error::Validation("test validation".into());
        let unified: Error = core.clone().into();
        assert!(matches!(
            unified,
            Error::Core(finstack_quant_core::Error::Validation(_))
        ));

        // Round-trip back to core should pass through unchanged
        let back: finstack_quant_core::Error = unified.into();
        assert_eq!(back, core);
    }

    #[test]
    fn core_error_propagates_with_question_mark() {
        fn inner() -> std::result::Result<(), finstack_quant_core::Error> {
        ...
        fn outer() -> std::result::Result<(), Error> {
            inner()?; // This requires From<finstack_quant_core::Error> for Error
            Ok(())
        }
    ...
    fn unified_converts_to_core_error() {
        ...
        let core_err: finstack_quant_core::Error = Error::Pricing(pricing).into();
        assert!(matches!(
            core_err,
