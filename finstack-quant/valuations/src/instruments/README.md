...
Instrument types, shared traits, pricing models, and JSON loading for `finstack-quant-valuations`.
...
Instruments load through `InstrumentEnvelope` (`json_loader.rs`) with schema `finstack_quant.instrument/1`. Unknown fields are rejected at deserialize time.
