    ...
    fn validate(&self) -> Result<()> {
        ...
        if self.underlying_ticker.trim().is_empty() {
            return Err(finstack_quant_core::Error::Validation(
                "VarianceSwap requires a non-empty underlying_ticker".to_string(),
            ));
        }
        validation::validate_money_finite(self.notional, "VarianceSwap notional")?;
        validation::validate_money_gt(self.notional, 0.0, "VarianceSwap notional")?;
        validation::validate_f64_non_negative(
            self.strike_variance,
            "VarianceSwap strike_variance",
        )?;
        ...
        finstack_quant_core::dates::calendar_by_id(&self.observation_calendar_id).ok_or_else(
            || {
                finstack_quant_core::Error::Validation(format!(
                    "VarianceSwap '{}' references unknown observation_calendar_id '{}'",
                    self.id, self.observation_calendar_id
                ))
            },
        )?;
        for (name, id) in [
            ("open_series_id", self.open_series_id.as_deref()),
            ("high_series_id", self.high_series_id.as_deref()),
            ("low_series_id", self.low_series_id.as_deref()),
            ("close_series_id", self.close_series_id.as_deref()),
        ] {
            if id.is_some_and(|value| value.trim().is_empty()) {
                return Err(finstack_quant_core::Error::Validation(format!(
                    "VarianceSwap {name} must not be empty when supplied"
                )));
            }
        }
        ...
        if self
            .settlement_date
            .is_some_and(|settlement| settlement < final_observation)
        {
            return Err(finstack_quant_core::Error::Validation(format!(
                "VarianceSwap '{}' settlement date precedes final observation ({final_observation})",
                self.id
            )));
        }
        Ok(())
