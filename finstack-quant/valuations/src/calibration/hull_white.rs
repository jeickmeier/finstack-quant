...
/// Generalized Hull-White parameters with a piecewise-constant short-rate
/// volatility schedule.
///
/// [`HullWhiteParams`] remains the scalar compatibility type. New
/// time-dependent consumers should accept this wrapper and scalar callers can
/// convert through [`From`].
#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
pub struct HullWhiteModelParams {
    /// Mean-reversion speed κ.
    pub kappa: f64,
    /// Left-continuous short-rate volatility σ(t).
    pub volatility: PiecewiseConstantCurve,
}

impl HullWhiteModelParams {
    /// Build a model with validated mean reversion and volatility schedule.
    pub fn new(
        kappa: f64,
    ...
    /// Build a constant-volatility model from scalar parameters.
    pub fn constant(kappa: f64, sigma: f64) -> finstack_quant_core::Result<Self> {
        Self::new(kappa, PiecewiseConstantCurve::constant(sigma)?)
    }

    /// State variance `Var[x(t)]` under the scheduled volatility.
    pub fn state_variance(&self, t: f64) -> finstack_quant_core::Result<f64> {
        self.volatility
            .integrate_squared_exp_weight(self.kappa, t, 0.0, t)
    }

    /// Covariance of the centered OU state at two times.
...
impl TryFrom<HullWhiteParams> for HullWhiteModelParams {
    ...
    fn try_from(params: HullWhiteParams) -> Result<Self, Self::Error> {
