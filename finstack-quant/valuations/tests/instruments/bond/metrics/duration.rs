...
fn test_convexity_matches_numerical_second_derivative() {
    ...
    let dy = 1e-4;
    let p0 = finstack_quant_valuations::instruments::fixed_income::bond::pricing::quote_conversions::price_from_ytm(
        &bond, &flows, quote_date, ytm,
    )
    .unwrap();
    let p_up = finstack_quant_valuations::instruments::fixed_income::bond::pricing::quote_conversions::price_from_ytm(
        &bond,
        &flows,
        quote_date,
        ytm + dy,
    ...
    .unwrap();
    let p_dn = finstack_quant_valuations::instruments::fixed_income::bond::pricing::quote_conversions::price_from_ytm(
        &bond,
        &flows,
        quote_date,
        ytm - dy,
