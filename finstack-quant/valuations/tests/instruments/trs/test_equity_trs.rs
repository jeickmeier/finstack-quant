...
fn test_equity_trs_financing_leg_matches_provider_schedule() {
    ...
    for (flow, payment_end) in coupon_flows.iter().zip(payment_dates.dates.iter().skip(1)) {
        ...
        let payment_date = trs
            ...
            .expect("payment date");
        let df = finstack_quant_valuations::instruments::pricing::time::relative_df_discount_curve(
            discount.as_ref(),
            as_of,
            payment_date,
        )
...
fn test_equity_trs_sensitivity_to_interest_rates() {
    ...
    let fwd_shifted = finstack_quant_core::market_data::term_structures::ForwardCurve::builder(
        "USD-SOFR-3M",
        0.25,
    )
    .base_date(as_of)
