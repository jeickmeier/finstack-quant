...
//! semi-annual, 2 settlement days — the defaults of finstack-quant's `Bond::fixed`
...
//! `finstack-quant/attribution/tests/attribution/quantlib_parity.rs`; both tests
...
use finstack_quant_core::Result as CoreResult;
use finstack_quant_valuations::instruments::fixed_income::bond::Bond;
use finstack_quant_valuations::instruments::fx::FxForward;
...
fn external_quantlib_parity_vanilla_bond_npv_and_dv01() {
    ...
    // finstack-quant's full DV01 metric pipeline is exercised elsewhere; here we
...
/// finstack-quant's `usd_irs_swap` helper uses `StubKind::None` and projects the
...
/// finstack-quant's IRS pricer agrees with QL in approach but too large to assert
