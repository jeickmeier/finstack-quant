...
use finstack_quant_core::dates::DateExt; // Imports DateExt
...
fn test_all_quote_types_instrument_construction() {
    ...
    let build_ctx =
        finstack_quant_valuations::market::BuildCtx::new(base_date, 1_000_000.0, curve_ids);

    for q in &discount_quotes {
        if let MarketQuote::Rates(rq) = q {
            finstack_quant_valuations::market::build_rate_instrument(rq, &build_ctx)
    ...
    for q in &hazard_quotes {
        if let MarketQuote::Cds(cds_q) = q {
            finstack_quant_valuations::market::build_cds_instrument(cds_q, &build_ctx)
