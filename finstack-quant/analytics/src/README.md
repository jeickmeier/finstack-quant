...
- Sample statistics (`n - 1`) match Bloomberg/QuantLib-style conventions; population variance lives in `finstack_quant_core::math::stats` when needed.
...
| Need | Crate |
|------|-------|
| Analytics on `Vec<f64>` return panels | `finstack-quant-analytics` |
| Python analytics on Polars frames | `finstack-quant-py` (`Performance`) |
| Realized volatility from OHLC | `finstack_quant_core::math::stats` |
| Instrument pricing or model greeks | `finstack-quant-valuations` |
| Cashflow NPV or IRR | `finstack_quant_core::cashflow` |
| Curves and discount factors | `finstack_quant_core::market_data` |
