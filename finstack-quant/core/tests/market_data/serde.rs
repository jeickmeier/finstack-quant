...
fn market_context_state_is_deterministically_sorted_and_roundtrips_full_snapshot() {
    ...
    let mut issuer_recovery = finstack_quant_core::HashMap::default();
    issuer_recovery.insert("ISSUER1".to_string(), 0.35);
    let mut issuer_weights = finstack_quant_core::HashMap::default();
    ...
    let ids: Vec<String> = state
        ...
        .map(|c| match c {
            ...
            finstack_quant_core::market_data::context::CurveState::Hazard(hc) => {
                hc.id().to_string()
            }
            finstack_quant_core::market_data::context::CurveState::Inflation(ic) => {
                ic.id().to_string()
            }
            finstack_quant_core::market_data::context::CurveState::BaseCorrelation(bc) => {
                bc.id().to_string()
            }
            finstack_quant_core::market_data::context::CurveState::VolIndex(vc) => {
                vc.id().to_string()
            }
            finstack_quant_core::market_data::context::CurveState::Price(pc) => pc.id().to_string(),
            finstack_quant_core::market_data::context::CurveState::BasisSpread(bs) => {
                bs.id().to_string()
            }
            finstack_quant_core::market_data::context::CurveState::Parametric(pc) => {
                pc.id().to_string()
            }
        })
        .collect();
...
fn curve_storage_roundtrip_and_market_context_state_error_branch() {
    use finstack_quant_core::market_data::context::{CurveState, CurveStorage};
    ...
    let bad_state = MarketContextState {
        ...
        dividends: vec![],
        credit_indices: vec![
            finstack_quant_core::market_data::context::CreditIndexState {
                id: "CDX".to_string(),
                num_constituents: 125,
                recovery_rate: 0.4,
