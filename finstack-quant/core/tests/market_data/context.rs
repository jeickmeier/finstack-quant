...
use finstack_quant_core::market_data::context::{BumpSpec, CurveStorage, MarketContext};
...
fn market_context_supports_curve_bumps() {
    ...
    let bumped = ctx
        .bump([MarketBump::Curve {
            ...
            spec: finstack_quant_core::market_data::context::BumpSpec::parallel_bp(50.0),
        }])
    ...
    let bumped = ctx
        .bump([
            ...
            MarketBump::Curve {
                ...
                spec: BumpSpec {
                    mode: finstack_quant_core::market_data::context::BumpMode::Multiplicative,
                    units: finstack_quant_core::market_data::context::BumpUnits::Factor,
                    value: 1.05,
            ...
            MarketBump::Curve {
                id: CurveId::from("TS"),
                spec: BumpSpec {
                    mode: finstack_quant_core::market_data::context::BumpMode::Additive,
                    units: finstack_quant_core::market_data::context::BumpUnits::Percent,
                    value: 10.0,
                    bump_type: BumpType::Parallel,
                },
            },
...
fn inflation_key_rate_bump_preserves_curve_metadata() {
    let curve =
        finstack_quant_core::market_data::term_structures::InflationCurve::builder("US-CPI")
            .base_cpi(100.0)
    ...
    assert_eq!(
        ...
        finstack_quant_core::math::interp::InterpStyle::Linear
    );
}
...
fn market_context_roll_forward_observed_propagates_errors_and_metadata() {
    ...
    let sparse =
        finstack_quant_core::market_data::term_structures::DiscountCurve::builder("SPARSE")
            .base_date(sample_base_date())
...
fn market_context_insert_and_stats_setters_cover_remaining_paths() {
    ...
    let surface_by_value =
        finstack_quant_core::market_data::surfaces::VolSurface::builder("IR-VOL")
            .expiries(&[1.0, 2.0])
            .strikes(&[90.0, 100.0])
            .row(&[0.2, 0.2])
            .row(&[0.2, 0.2])
