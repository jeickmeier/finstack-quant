...
The `cashflow` module in `finstack-quant-core` provides dated cashflow and present
...
```rust
use finstack_quant_core::cashflow::discounting::{npv, Discountable};
...
```rust
use finstack_quant_core::cashflow::{xirr, xirr_with_daycount};
use finstack_quant_core::dates::DayCount;
use time::macros::date;
