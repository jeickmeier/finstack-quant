...
The `dates` module in `finstack-quant-core` provides time and calendar primitives for
...
[`finstack_quant_core::math::Compounding::convert_rate`](../math/compounding.rs).

Everything is accessible via `finstack_quant_core::dates`:
...
[`finstack_quant_core::math::Compounding`](../math/compounding.rs), not in `dates`.
...
- With `.error_policy(ScheduleErrorPolicy::GracefulEmpty)`, `build()` returns an **empty** schedule with a `ScheduleWarning::GracefulFallback` warning instead of an error. Always check `schedule.has_warnings()` to detect suppressed errors and avoid silent PV=0 scenarios.
...
```rust
use finstack_quant_core::dates::{Tenor, TenorUnit, Date, DayCount, DayCountContext, BusinessDayConvention};
...
```rust
use finstack_quant_core::dates::{FiscalConfig, build_fiscal_periods};
...
```rust
use finstack_quant_core::dates::{third_wednesday, next_imm, next_cds_date, next_imm_option_expiry};
use time::{Date, Month};
...
```rust
use finstack_quant_core::dates::{Date, DateExt, BusinessDayConvention, adjust};
use finstack_quant_core::dates::calendar::TARGET2;
use time::Month;
...
```rust
use finstack_quant_core::dates::{build_periods, build_fiscal_periods, FiscalConfig};
...
Calendars: JSON under `data/calendars/`, rebuild, tests in `tests/dates/`.
Day-count: new `DayCount` variant in `daycount.rs` with `DayCountContext` support.
Schedules: extend `ScheduleBuilder` in `schedule_iter.rs`; preserve strictly
increasing dates. Rate conversions belong in `math::Compounding`, not `dates`.
