...
The `math` module in `finstack-quant-core` provides deterministic numerical building
...
`finstack_quant_core::math` re-exports the common entry points from `mod.rs`:
...
```rust
use finstack_quant_core::math::solver_multi::{LevenbergMarquardtSolver, MultiSolver};
...
```rust
use finstack_quant_core::math::interp::{LogLinearDf, ExtrapolationPolicy};
...
```rust
use finstack_quant_core::math::{mean, variance, covariance, correlation};
