...
`finstack-quant-core` depends on **no** other workspace crate. Domain crates consume it, as do `finstack-quant-arrow`,
...
| `market_data::surfaces` | `VolSurface`, `VolCube`, `FxDeltaVolSurface` (validated ATM / risk-reversal / butterfly quotes; conversion lives in `finstack-quant-models::volatility`). |
...
`finstack-quant-models` or `valuations`.
...
`HashMap`/`HashSet` are re-exported at the crate root as `rustc_hash` aliases
with deterministic hashing. Use sorted maps or explicit sorting for canonical
serialization; hash-map iteration is not a canonical ordering.
...
generator. Hash containers use `rustc_hash` aliases for deterministic hashing.
...
|------|----------------|-------------|
| `currency_generated.rs` (`Currency` enum, `MINOR_UNITS`) | `build/generate_reference_data.py`; included directly by `src/currency.rs` | `data/iso_4217.csv` |
| `cny_generated.rs` (Chinese New Year dates) | `build/generate_reference_data.py` | `data/chinese_new_year.csv` |
| `festivals_generated.rs` (Dragon Boat, Mid-Autumn) | `build/gen_lunar_festivals.py`, which rewrites the Rust table and both CSVs | `data/dragon_boat.csv`, `data/mid_autumn.csv` |

`currency_generated.rs` is `include!`d from `src/currency.rs`. `cny_generated.rs`
and `festivals_generated.rs` are `include!`d from `src/dates/calendar/algo.rs`.
Regenerate currency and Chinese New Year tables with `mise run core-gen-data`;
verify them with `mise run core-check-data`. Those tasks are included in
`gen-write` and `gen-check`. Refresh Dragon Boat and Mid-Autumn tables with
`build/gen_lunar_festivals.py`. Maintain the CSV inputs and regenerate the Rust
tables rather than editing both copies by hand.
...
mise run rust-lint-crate -- finstack-quant-core
mise run rust-doc
