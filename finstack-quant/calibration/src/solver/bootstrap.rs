...
fn validate_time_ordering(time: f64, last_time: f64, original_idx: usize) -> Result<()> {
    ...
    if (time - last_time).abs() <= crate::constants::TOLERANCE_DUP_KNOTS {
        return Err(finstack_quant_core::Error::Calibration {
            message: format!(
...
impl SequentialBootstrapper {
    ...
    ///
    /// # Generic Parameters
    /// * `T` - The calibration target (e.g., [`DiscountCurveTarget`](crate::targets::discount::DiscountCurveTarget)).
    ///
    /// # Arguments
    ...
    /// * `tolerance` - Target-specific validation tolerance for determining calibration success.
    ///
    /// # Returns
    ...
    {
        let validation_tolerance = tolerance;
...
mod tests {
    ...
    fn bootstrap_is_order_invariant() -> Result<()> {
        ...
        let (curve_a, _) = SequentialBootstrapper::bootstrap(
            &target,
            &quotes,
            Vec::new(),
            &config,
        ...
        let (curve_b, _) = SequentialBootstrapper::bootstrap(
            &target,
            &shuffled,
            Vec::new(),
            &config,
...
mod w40_tests {
    ...
    impl BootstrapTarget for NonMonotoneTarget {
        ...
        fn quote_time(&self, quote: &Self::Quote) -> finstack_quant_core::Result<f64> {
            Ok(quote.t)
        }
    ...
    fn w40_no_bracket_approximate_knot_is_flagged_in_report() {
        ...
        let result =
            SequentialBootstrapper::bootstrap(&target, &[q], vec![(0.0, 0.0)], &cfg, tolerance);
    ...
    fn item10_no_bracket_knot_is_a_hard_failure_not_a_silent_success() {
        ...
        let result = SequentialBootstrapper::bootstrap(&target, &[q], vec![(0.0, 0.0)], &cfg, 1e-6);
