...
struct BondExpectedAttribution {
    ...
    /// test bounds finstack-quant's residual by it: the metrics path includes the
...
/// Wide enough to absorb minor differences in finstack-quant's metric calculator
...
fn quantlib_parity_metrics_based_bond_attribution() {
    ...
    // Carry parity: finstack-quant's `Theta × days` vs QL's 1-day theta.
    ...
    // Rates parity: finstack-quant's `DV01 × Δr_bp` (or key-rate sum) vs QL's
    // `dv01 × Δrate_bp`.
    ...
    // Sanity: finstack-quant's total_pnl matches QL's actual_pnl within the
...
fn flat_forward_curve(
    ...
    base: Date,
    rate: f64,
) -> finstack_quant_core::market_data::term_structures::ForwardCurve {
    finstack_quant_core::market_data::term_structures::ForwardCurve::builder(id, 0.25)
        .base_date(base)
...
/// because `finstack-quant-attribution`'s dev-dependencies do not include the
...
fn build_irs(
    ...
    use finstack_quant_core::dates::{BusinessDayConvention, DayCount as DC, StubKind, Tenor};
    ...
    use finstack_quant_valuations::instruments::InterestRateSwap;
...
impl finstack_quant_core::money::fx::FxProvider for FixedEurUsd {
    fn rate(
        &self,
        from: Currency,
        to: Currency,
        _on: Date,
        _policy: finstack_quant_core::money::fx::FxConversionPolicy,
...
fn quantlib_parity_metrics_based_fx_forward_attribution() {
    ...
    use finstack_quant_valuations::instruments::FxForward;
