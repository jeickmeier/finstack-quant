...
pub use finstack_quant_core::rating_scales::{RatingLevel, ScorecardScale};
...
fn get_rating_scale(scale_name: &str) -> Result<&'static ScorecardScale> {
    Ok(finstack_quant_core::rating_scales::embedded_registry()?.rating_scale(scale_name)?)
}

fn is_supported_rating_scale(scale_name: &str) -> bool {
    finstack_quant_core::rating_scales::embedded_registry()
...
impl CreditScorecardExtension {
    ...
    pub fn validate_config(config: &ScorecardConfig) -> Result<()> {
        ...
        if let Some(period) = &config.period {
            period
                .parse::<finstack_quant_core::dates::PeriodId>()
                .map_err(|e| {
                    finstack_quant_statements::error::Error::invalid_input(format!(
    ...
    ) -> Result<&'m finstack_quant_core::dates::Period> {
        if let Some(period_str) = &config.period {
            let pid: finstack_quant_core::dates::PeriodId = period_str.parse().map_err(|e| {
                finstack_quant_statements::error::Error::invalid_input(format!(
    ...
    fn evaluate_metric(
        ...
        let mut eval_context = finstack_quant_statements::evaluator::EvaluationContext::new(
            target_period.id,
