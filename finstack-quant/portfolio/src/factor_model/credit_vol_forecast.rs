...
//! model artifact ([`CreditFactorModel`] from `finstack-quant-core`) and the
...
//! `finstack-quant/valuations/src/factor_model/credit_vol_forecast.rs`. The
//! `factor_model_at` helper, however, returns a [`crate::factor_model::FactorModel`], which lives
//! in `finstack-quant-portfolio` (and `finstack-quant-portfolio` already depends on
//! `finstack-quant-valuations`, so the reverse dependency is not available). To
//! keep the API surface intact without inverting the dependency graph the
//! whole module landed here in `finstack-quant-portfolio` instead. The covariance
//! and idiosyncratic-vol helpers only use `finstack-quant-core` types and would
//! work just as well from `finstack-quant-valuations`; relocation is a low-cost
...
use finstack_quant_valuations::Error as ValuationsError;
    ...
    pub fn covariance_at(
        ...
        for fid in &factor_ids {
            let vol_model = self.model.vol_state.factors.get(fid).ok_or_else(|| {
                ValuationsError::Core(finstack_quant_core::Error::Validation(format!(
    ...
    pub fn factor_model_at(
        ...
        FactorModelBuilder::new()
            ...
            .map_err(|e: PortfolioError| {
                ...
                // a single error type.
                ValuationsError::Core(finstack_quant_core::Error::Validation(e.to_string()))
            })
    }
