...
pub struct Evaluator {
    ...
    visibility_cutoff: Option<finstack_quant_core::dates::Date>,
}
...
impl Evaluator {
    ...
    fn evaluate_inner(
        &mut self,
        model: &FinancialModelSpec,
        market_ctx: Option<&finstack_quant_core::market_data::context::MarketContext>,
        as_of: Option<finstack_quant_core::dates::Date>,
    ) -> Result<StatementResult> {
        ...
        if model.capital_structure.is_some() {
            return Err(Error::eval(
                "Monte Carlo evaluation for statements does not yet support capital_structure. \
                 Run Monte Carlo on the underlying instruments using finstack-quant-valuations \
