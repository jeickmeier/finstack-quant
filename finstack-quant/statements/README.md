...
Higher-level analysis (DCF, scenarios, scorecards, covenants) lives in `finstack-quant-statements-analytics`.
...
```bash
cargo test -p finstack-quant-statements
cargo bench -p finstack-quant-statements --bench statements_operations --no-run
```
