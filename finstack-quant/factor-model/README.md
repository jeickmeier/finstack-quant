...
Canonical multi-asset factor-modelling primitives for finstack_quant. This crate owns
...
- Portfolio sensitivity/risk engines in `finstack-quant-portfolio` that consume the
...
sensitivity engines in `finstack-quant-portfolio::sensitivity`. The crate provides
...
`finstack-quant-portfolio` because they take `&dyn Instrument`.

## Related

- `finstack-quant-core` — base primitives consumed by factor models (`Currency`,
  `Date`, `CurveId`, `IssuerId`, market-data bump units, and errors).
- `finstack-quant-analytics` — `beta` OLS slope and `nearest_correlation_matrix`
  PSD repair used by the calibrator.
- `finstack-quant-portfolio::sensitivity` — sensitivity engines that consume the
  calibrated model and produce a [`SensitivityMatrix`].
- `finstack-quant-valuations` — instrument pricing that consumes the calibrated
