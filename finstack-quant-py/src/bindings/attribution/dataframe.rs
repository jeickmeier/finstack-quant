...
pub(super) fn build_long_detail_rows(
    attribution: &finstack_quant_attribution::PnlAttribution,
) -> Vec<LongDetailRow> {
    ...
    if let Some(detail) = &attribution.model_params_detail {
        let mut push_opt =
            |key: &'static str, money: &Option<finstack_quant_core::money::Money>| {
                if let Some(m) = money {
                    rows.push(LongDetailRow {
...
pub(super) fn build_carry_detail_rows(
    ...
    let mut push = |kind: &'static str, key_a: &str, money: &finstack_quant_core::money::Money| {
        rows.push(LongDetailRow {
            kind,
            factor: "carry",
