...
in [`finstack-quant-valuations`] and produce JSON that ``calibrate`` and ``dry_run``
accept verbatim.
...
class Tenor(TypedDict):
    ...
    Maps to ``finstack_quant_core::dates::Tenor``. Serializes as
    ``{"count": 5, "unit": "years"}``.
    """
...
CalibrationEnvelope = TypedDict(
    "CalibrationEnvelope",
    {
        ...
        "schema": Literal["finstack_quant.calibration"],
