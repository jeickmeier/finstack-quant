# Quant Finance Review — `finstack-quant/attribution` Crate and Bindings
...
**Date:** 2026-06-12
**Scope:** `finstack-quant/attribution` (~12.7k source lines: parallel/waterfall/Taylor/metrics-based methodologies, credit cascade/factor decomposition, market snapshots, target-currency translation, spec/envelope types), `finstack-quant-py/src/bindings/attribution/`, `finstack-quant-wasm/src/api/attribution/` + JS facade, `parity_contract.toml` attribution sections, and the crate's ~8.8k-line test/bench surface (incl. `quantlib_parity.rs`).
...
- **Location:** `finstack-quant/attribution/src/parallel.rs:46-56` (`cross_interaction_pnl`), `:128-138` (`record_cross_pair`), `:693-775` (full-cross path), `:963-1062` (default 6-pair path); consumed by `finstack-quant/attribution/src/types/result.rs:664-669` (`compute_residual`).
...
- **Location:** `finstack-quant/attribution/src/factors.rs:216-277` (`extract`), `:291-384` (`restore_market`) vs `finstack-quant/core/src/market_data/context/mod.rs:148-185, 457-477`.
...
- **Location:** `finstack-quant/attribution/src/metrics_based.rs:940-963` (formula), `:929-938` (`Convexity`/`IrConvexity` merged via `.or_else`).
...
- **Location:** `finstack-quant/attribution/src/credit_factor.rs:215, 229, 239` (accumulations), `:9` and `:25-27` (module identity), `:76-79` (input doc); test `tests/attribution/credit_factor_linear.rs:154-175`.
...
- **Location:** `types/detail.rs:103, 121, 134, 152` (`IndexMap<(CurveId, String), Money>` / `IndexMap<(Currency, Currency), Money>` with derived `Serialize`); surfaced at `finstack-quant-py/.../entry.rs:92`, `dataframe.rs:34-138`; roundtrip-test gap at `tests/attribution/serialization_roundtrip.rs:351-375`.
...
- **Location:** `finstack-quant-py/src/bindings/module_utils.rs:127-130` (via `attribution/entry.rs:67`).
...
- **Location:** `finstack-quant-py/src/bindings/attribution/entry.rs:80, 91, 119-120, 145`; `pnl_attribution.rs:266`.
...
- **Fix:** Use `core_to_py` wherever the error type is `finstack_quant_core::Error`; add tests asserting `KeyError` for a missing curve and `RuntimeError` for internal failures.
...
- **MO-B1. `validate_attribution_json` doesn't check `ATTRIBUTION_SCHEMA_V1`** (Python and WASM): a `"finstack_quant.attribution/99"` envelope passes validation and then fails at execute — `entry.rs:142-146`, `wasm mod.rs:197-202`.
