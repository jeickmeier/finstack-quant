# Quant Review — `finstack-quant-analytics` + bindings

**Date:** 2026-06-09
**Remediation status (2026-06-11):** All findings implemented except M10 (external golden tests vs quantstats/PerformanceAnalytics — deferred, needs externally generated reference fixtures) and the centered-Welford rolling-greeks rewrite (documented trade-off retained). Notable intentional behavior changes: zero-variance benchmark now yields NaN beta (was 0.0) across `beta`/`beta_only`/`greeks`; Python/WASM `dates()` now returns the full grid per Rust semantics with `active_dates()`/`activeDates()` added; `DrawdownEpisode.start/valley/end` and `RollingGreeks.dates`/`DatedSeries.dates` are now Python properties (were methods); `lookback_returns` gained `fiscal_year_start_day` and `calendar` parameters (defaults preserve prior numbers).
**Scope:** All 23 source files in `finstack-quant/analytics` (~7.5k source lines), the core math it delegates to (`finstack-quant/core/src/math/stats.rs`, `finstack-quant/core/src/dates/periods.rs`), `finstack-quant-py/src/bindings/analytics`, `finstack-quant-wasm/src/api/analytics`, `.pyi` stubs, `parity_contract.toml`, and the test suites.
...
Rust has `dates()` (full constructed grid, `performance/mod.rs:597-599`) and `active_dates()` (windowed, `performance/mod.rs:548-552`); both bindings expose a single `dates()` that returns `active_dates()` (`finstack-quant-py/src/bindings/analytics/performance.rs:337-344`, `finstack-quant-wasm/src/api/analytics/performance.rs:225-233`). After `reset_date_range`, code ported from Rust silently changes meaning, and the full grid is unreachable from Python/JS. Violates the project's "names match Rust exactly" canonical-API rule.
...
`finstack-quant-py/parity_contract.toml:95-115` records only module-level `status = "flattened"` notes. `test_contract_symbols_match_live_surface` iterates `CRATES_WITH_SYMBOLS` only, so the 10-name analytics surface can drift without CI noticing — unlike `valuations`/`attribution`, which have `symbols.public` lists.
...
`RollingGreeks.alphas/betas`, `MultiFactorResult.betas`, `LookbackReturns.mtd/qtd/ytd/fytd`, `DatedSeries.values` all return `PyArray1<f64>` (`finstack-quant-py/src/bindings/analytics/types.rs:200-417`) but are typed `list[float]` in the stub (`.pyi:147-153, 168-170, 219-233, 251-253`). Type-checked user code (`.append`, `isinstance(x, list)`, JSON-serialize) passes mypy and breaks at runtime. The scalar-metric methods genuinely return `list[float]`, so the stub is inconsistent within the same module.
...
`finstack-quant-py/src/bindings/analytics/performance.rs:750-795`. Sortino, downside deviation, and Omega are pinned at 0.0 regardless of the `risk_free_rate` argument, undocumented; and the 22-metric selection lives only in the Python binding (no Rust or WASM counterpart), so it can drift per language.
...
- **Bindings nits** (file/line refs verified by the parity sweep):
  ...
  - TS declares `DatedSeries.values?: Float64Array` (`index.d.ts:404-411`) which the WASM payload never emits — `dated_series_to_js` (`finstack-quant-wasm/src/api/analytics/performance.rs:119-128`) only emits the metric-named key; also diverges from Python's `DatedSeries` shape.
  ...
  - `py_to_date` duck-typing (`finstack-quant-py/src/bindings/core/dates/utils.rs:8-15`) raises a bare `AttributeError` for ISO strings instead of `TypeError`, and silently accepts tz-aware timestamps' wall-clock date with no documented normalization. WASM's `parse_iso_date` is stricter and clearer.
