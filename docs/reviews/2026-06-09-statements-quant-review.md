...
**Date:** 2026-06-09
**Scope:** `finstack/statements`, `finstack/statements-analytics` (plus the `finstack/covenants` crate the credit bridge delegates to), `finstack-py/src/bindings/{statements,statements_analytics}`, `finstack-wasm/src/api/{statements,statements_analytics}`, `.pyi` stubs, JS facade, parity contract.
...
**B1 — Stage 3 ECL is priced with the performing-rating PD curve, not PD = 1** — `finstack/statements-analytics/src/analysis/ecl/engine.rs:418-449`
...
**B2 — Off-by-one in the cashflow scale denominator silently inflates interest on on-schedule amortizing loans** — `finstack/statements/src/capital_structure/period_flows.rs:93-104`
...
**B3 — `project_finance` covenant package: two `MinDSCR` specs share one identity; a distribution-lockup breach triggers an Event of Default** — `finstack/covenants/src/templates.rs:186-211`
...
**B4 — Comps regression silently misaligns x/y vectors when peers have partially missing data** — `finstack/statements-analytics/src/analysis/comps/scoring.rs:105-121`
...
- **`CeclEngine` with empty `pd_sources` silently returns ECL = 0** — `finstack/statements-analytics/src/analysis/ecl/cecl.rs:146-201`; the IFRS-9 path rejects this, CECL doesn't. **Fix:** validate non-empty in `CeclEngine::new`.
...
- **Gordon/H-Model terminal value capitalizes the last *period* flow with *annual* WACC and g** — `finstack/statements-analytics/src/analysis/valuation/corporate.rs:254-280`, `finstack/valuations/src/instruments/equity/dcf_equity/types.rs:504-532`. With this crate's default quarterly models, TV (typically 60-80% of EV) is understated ~4×. **Fix:** annualize the terminal flow (trailing 12m), or validate the period grid is annual for growth-perpetuity terminal specs.
...
**Bindings**
- Python `score_relative_value` re-implements input mapping in the binding (hardcoded `PeriodBasis::Ltm`, invented dimension mini-language, `MetricExtractor::Multiple` unreachable) while WASM takes canonical serde shapes — `finstack-py .../comps.rs:167-349` vs `finstack-wasm .../comps.rs:100-107`.
- Python `peer_stats` renames `count`→`n` and computes `iqr` in the binding (cross-binding drift; logic in binding) — `finstack-py .../comps.rs:84,88`.
- `compute_ecl` patches the PD curve's `(0,0)` anchor in the binding but `compute_ecl_weighted` doesn't — identical curve accepted vs `ValueError` — `finstack-py .../ecl.rs:285-294` vs `:396-426`. Move anchoring into a Rust helper.
...
- `to_pandas_long` docstring overclaims Decimal precision — `value_money` comes from the f64 mirror (`export.rs:38`), only `to_json()` preserves fixed-point — `finstack-py .../statements/evaluator.rs:106-112`.
...
- WASM comps: `null` vs `undefined` inconsistency contradicting `index.d.ts` `| null` declarations — `finstack-wasm .../comps.rs`.
...
- B3 lives in `finstack-covenants`, which the statements-analytics credit bridge re-exports/delegates to; treated as in scope since the exposed credit workflow routes through it.
