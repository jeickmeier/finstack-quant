# Quant Review — `finstack-cashflows` + bindings (re-review)

**Date:** 2026-06-13
**Scope:** `finstack/cashflows` crate and its Python/WASM bindings.
...
`finstack/cashflows/src/accrual.rs:204` (`AccrualConfig`), `:150` (`ExCouponRule`).

Both are deserialized directly by `accrued_interest_json` (the Python/WASM bridge) yet neither carries `#[serde(deny_unknown_fields)]`. A misspelled `method`, `ex_coupon`, or `days_before_coupon` key is silently ignored and the financially-significant default is used. This is **provably live**: `finstack-py/tests/test_cashflows.py:163` passes `"strict_issue_date": True` — a field deleted from Rust in `d8981646f` — and the test still asserts success only because the key is silently dropped.
...
`finstack/cashflows/src/builder/emission/coupons.rs:826`.
...
`finstack/cashflows/src/builder/emission/coupons.rs:776-788`.
...
`finstack/cashflows/src/builder/dataframe.rs:606-616` vs `finstack/cashflows/src/aggregation.rs:501-534`.
...
`finstack/cashflows/src/builder/dataframe.rs:502-551`.
...
`finstack/cashflows/src/accrual.rs:606-610`.
...
`finstack/cashflows/src/builder/pipeline.rs:100-113`.
...
`finstack/cashflows/src/builder/periods.rs:147-230`.
...
- **No schema_version / rounding-context stamp** in the wire format — still open from M13 (`finstack/cashflows/src/json.rs:153`).
- **`CashFlow` (core) and `TenorDe` (core) lack `deny_unknown_fields`** — transitively inbound via the bridge (`finstack/core/src/cashflow/primitives.rs:317`, `finstack/core/src/dates/tenor.rs:162`); required fields still error, so only additive junk is tolerated.
...
- **`nth_tenor` integer multiply unguarded against i32 overflow + no anchor-count cap** (`finstack/core/src/dates/schedule_gen.rs:226`) — robustness only; parsed/deserialized tenors are capped.
...
- **Python `__all__` registration order not alphabetized** (`finstack-py/src/bindings/cashflows/mod.rs:172`) — disagrees with the stubs; project standard says sorted.
