...
**Fix.** Extract the name from the normalized string (split on the first '_' or ':' of the normalized form, preserving original casing via index arithmetic), or split the original on any of [':','_','-',' ','/']; reject empty custom names with an error.
