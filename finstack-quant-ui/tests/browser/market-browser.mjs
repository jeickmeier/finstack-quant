...
try {
  ...
  await writeFile(
    ...
    await readFile(path.join(root, "tests/market-browser/cases.json")),
  );
  await writeFile(
  ...
  await writeFile(
    ...
    '<!doctype html><html lang="en"><head><meta charset="UTF-8"><title>Market browser</title><link rel="stylesheet" href="/app.css"></head><body class="finstack-surface"><div id="root"></div><script type="module" src="/main.tsx"></script></body></html>',
  );
  const modules = [];
  await build({
    root: consumer,
