...
try {
  ...
  for (const linked of [mode === "linked"]) {
    ...
    const entry = linked
      ? 'import { CurveLinkExample } from "./components/finstack/components/curve-link-example/curve-link-example"; createRoot(document.getElementById("root")!).render(<main><h1>Table and curve linking</h1><CurveLinkExample/><CurveLinkExample label="Independent stored curves"/></main>);'
      : 'import { FinstackTable } from "./components/finstack/primitives/finstack-table/finstack-table"; createRoot(document.getElementById("root")!).render(<main><h1>Independent table</h1><FinstackTable caption="Exact supplied values" data={[{id:"a",value:"1000000.123456789"}]} columns={[{id:"value",accessorKey:"value",header:"Value"}]} getRowId={row=>row.id}/></main>);';
    await writeFile(
